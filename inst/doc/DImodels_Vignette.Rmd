...
author: "Rafael de Andrade Moral, John Connolly and Caroline Brophy"
date: "`r Sys.Date()`"
...
1. Step 1 compared five DI models using F tests. The models each had a different form of species interactions and treatment was always included. The functional group model (FG) was the selected model. This assumes that pairs of species interact according to functional group membership. 
2. Step 2 provided a test for the treatment and indicated that the treatment, included as an additive factor, was significant and needed in the model (p < 0.0001).
3. Step 3 uses profile likelihood to estimate the non-linear parameter $\theta$ and tests whether or not it differs from one. (Each model in Step 1 was fitted assuming theta equalled one.) Theta was estimated to be 0.96814 and was not significantly different from one (p = 0.4572).
...
First, we create the FG pairwise interactions, the `DI_data_FG` function.
